...
import org.liberty.j.jagdtiger.service.FlightTenDayService;
...
@RestController
@CrossOrigin
public class FlightTenDayController {

    public String id_to_city = new String("{'BJS': '北京', 'CGQ': '长春', 'CKG': '重庆', " +
    ...
    @Autowired
    private FlightTenDayService fts;
    @RequestMapping(value = "/flight/getPrice",method = RequestMethod.GET)
    public Result getCityFlightPrice(@RequestParam String city) throws Exception {
        ...
        for (int i = 0;i<splt.length ;i ++) {
            ...
            ans.put("时间",tplt[0]);
            ans.put("价格",Double.valueOf(tplt[1]));
            Ans.add(ans);
        }
